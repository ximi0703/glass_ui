...
class TestSmoke:
    ...
    def set_address(self, phone, a_type):
        ...
        if phone(resourceId="com.upuphone.star.launcher:id/name", text="回家").exists:
            ...
            time.sleep(2)
            logging.info("输入成都东站")
            phone(resourceId="com.upuphone.star.launcher:id/keyWord").set_text("成都东站")
            time.sleep(1)
            logging.info("点击搜索结果第一个成都东站")
            phone(resourceId="com.upuphone.star.launcher:id/name", textContains="成都东站")[0].click()
            time.sleep(4)
            logging.info("检查是否添加成功家庭地址")
            if "成都东站" in phone(resourceId="com.upuphone.star.launcher:id/home_address").get_text():
    ...
    def test_douyin_glass_double_quit(self, driver, driver1):
        ...
        if status and not glass(resourceId="com.upuphone.star.launcher:id/time_view").exists:
            ...
            while not glass(resourceId="com.upuphone.star.launcher:id/time_view").exists:
                os.system("adb -s %s shell input keyevent 21" % glass.serial)
                logging.info("TP两次长按未退出抖音成功，再次尝试")
                os.system(
                    "adb -s %s shell input keyevent 4&&adb -s %s shell input keyevent 4" % (
                        glass.serial, glass.serial))
                time.sleep(2)
            logging.info("TP两次长按退出抖音成功")
        else:
    ...
    def test_kuaishou_glass_double_quit(self, driver, driver1):
        ...
        if status and not glass(resourceId="com.upuphone.star.launcher:id/time_view").exists:
            ...
            while not glass(resourceId="com.upuphone.star.launcher:id/time_view").exists:
                os.system("adb -s %s shell input keyevent 21" % glass.serial)
                logging.info("TP两次长按未退出快手成功，再次尝试")
                os.system(
                    "adb -s %s shell input keyevent 4&&adb -s %s shell input keyevent 4" % (
                        glass.serial, glass.serial))
                time.sleep(2)
            logging.info("TP两次长按退出快手成功")
        else:
    ...
    def test_glass_start_music_click_quit(self, driver, driver1):
        ...
        if get_current_music_before != get_current_music:
            ...
            logging.error(
                "眼镜端tp切换下一首失败；切换后：%s， 切换前: %s" % (get_current_music_before, get_current_music))
            get_screen_shot(glass)
            pytest.assume(False)
        ...
        if get_current_music_after != get_current_music_before and get_current_music_after == get_current_music:
            logging.info(
                "眼镜端tp切换上一首成功；切换后：%s， 切换前: %s" % (get_current_music_after, get_current_music_before))
        else:
            logging.error(
                "眼镜端tp切换上一首失败；切换后：%s， 切换前: %s" % (get_current_music_after, get_current_music_before))
            get_screen_shot(glass)
            pytest.assume(False)
